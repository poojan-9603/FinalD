...
const Dashboard = () => {
  ...
  const [lastUpdatedDate, setLastUpdatedDate] = useState(new Date().toLocaleDateString()); 
  const [lastFetchedDate, setLastFetchedDate] = useState(new Date().toLocaleDateString()); 
  ...
  const fetchUserData = async (userId) => {
    try {
        ...
        if (!userSnapshot.empty) {
            ...
            const category = userDoc.data().userCategory || 'Cautious'; 
            setUserCategory(category); 
            
            // Update Firestore with userCategory if it doesn't exist
            if (!userDoc.data().userCategory) {
  ...
  const checkWeather = async (source) => {
    try {
        ...
        const weatherConditionText = weather.condition.text;
        setWeatherPrediction(weatherConditionText); 

        let isFavorable = false;

        if (!userCategory) {
            userCategory = 'Cautious'; 
        }

        if (userCategory.trim() === 'Weather-Resilient') {
            isFavorable = true; 
        } else if (userCategory.trim() === 'Cautious') {
            isFavorable = !weatherConditionText.toLowerCase().includes('rain') && 
                          !weatherConditionText.toLowerCase().includes('storm');
        } else if (userCategory.trim() === 'Weather-Averse') {
            isFavorable = !weatherConditionText.toLowerCase().includes('rain') && 
                          !weatherConditionText.toLowerCase().includes('storm') && 
                          !weatherConditionText.toLowerCase().includes('overcast');
        }
